...
async function listVocabulary(req: NextRequest) {
  ...
  const [items, total] = await Promise.all([
    prisma.vocabulary.findMany({ where, take: limit, skip: offset, orderBy: { word: "asc" } }),
    prisma.vocabulary.count({ where }),
  ]);

  const mapped = items.map((v) => ({
    id: v.id, word: v.word, article: v.article, pos: v.pos, level: v.level,
    translation: (v as Record<string, unknown>)[`translation${lang.charAt(0).toUpperCase() + lang.slice(1)}`] || v.translationEn,
    exampleSentence: v.exampleSentence, exampleTranslation: v.exampleTranslation,
  }));

  return NextResponse.json({ items: mapped, total, limit, offset });
}

export const GET = withApiKey(listVocabulary, "read:vocabulary");
