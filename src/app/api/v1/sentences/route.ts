...
async function listSentences(req: NextRequest) {
  ...
  const [items, total] = await Promise.all([
    prisma.sentence.findMany({ where, take: limit, skip: offset, orderBy: { text: "asc" } }),
    prisma.sentence.count({ where }),
  ]);

  const mapped = items.map((s) => ({
    id: s.id, text: s.text, translation: s.translationEn, level: s.level, sourceName: s.sourceName,
  }));

  return NextResponse.json({ items: mapped, total, limit, offset });
}

export const GET = withApiKey(listSentences, "read:sentences");
